...
const RepositoryInfo = ({ repository }: { repository: IRepo }) => {
  return (
    <div className="flex flex-col gap-4 w-full h-full p-5">
      {repository.imageFileHash ? <ImageComponent
        className="h-64 w-64 rounded-md border-[3px] border-gray-200 mx-auto"
        src={`${process.env.NEXT_PUBLIC_PODSPACE_API}files/${repository.imageFileHash}`}
        alt={repository.name}
      /> : <div className="w-full flex justify-center"><FolderEmptyIcon /></div>}

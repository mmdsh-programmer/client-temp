...
async function fetchDocumentVersion(
  ...
  if (versionId) {
    return getPublishedDocumentVersion(accessToken!, documentId, versionId, documentPassword);
  }
...
export default async function PrivateSharePage({ params }: PublishContentPageProps) {
  try {
    ...
    if (!documentInfo?.hasPassword && !documentInfo?.hasWhiteList && !documentInfo?.hasBlackList) {
      ...
      return <RedirectPage redirectUrl={`/share/${name}/${id}/${publicSlug}`} />;
    }
    ...
    try {
      ...
      if (
        hasVersion &&
        version &&
        toEnglishDigit(version.versionNumber).replaceAll(/\s+/g, "-") !== versionNumber
      ) {
        return notFound();
      }
